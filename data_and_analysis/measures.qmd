...
library("pacman")
p_load(tidyverse, here, psych, corrplot, lme4, mirt, BayesFactor)
...
HMT_item_matrix <- read.csv(here::here("data_and_analysis", "data", 
                              "part4_WMC_Gf", 
                              "df_HMT.csv")) |>  
  filter(id %in% WMC_result$id) |> 
  select(-id, -no_correct) 
```
...
#### Testing predictions for normative probabilistic expressions X1-X2:

```{r}
t.test(df_noise_RQ1$x1_average,  mu = 0)
t.test(df_noise_RQ1$x1_average, mu = 0)$p.value
t.test(df_noise_RQ1$x1_average, mu = 0)$statistic
```

```{r}
t.test(df_noise_RQ1$x2_average, mu = 0)
t.test(df_noise_RQ1$x2_average, mu = 0)$p.value
t.test(df_noise_RQ1$x2_average, mu = 0)$statistic
```

```{r}
t.test(df_noise$x_average, mu = 0)
```
